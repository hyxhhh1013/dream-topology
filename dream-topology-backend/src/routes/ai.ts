...
import { DreamService } from '../queries';
...
// D2: LRU cache for embeddings to avoid re-computing frequently accessed embeddings
const embeddingCache = createLRU<string, number[]>({
  max: 500,
});
...
const embedSchema = z.object({
  dreamText: z.string().min(1, "dreamText is required"),
...
aiRouter.post('/analyze', async (c) => {
  try {
    ...
    if (!content) {
      throw new AIError('No content returned from AI');
    }
          ...
          if (dreamRecordId) {
            await prisma.dreamToSymbol.create({
              data: {
                ...
                personalized_meaning: symbol.archetype_meaning
              }
            });
          }
        }
    ...
    // D6: Integrate DreamService - enrich response with topology data
    try {
      const topologyData = await dreamService.getTopologyData(userId);
      return c.json({
        data: {
          ...parsedData,
          dreamRecordId,
      ...
      // If topology fetching fails, return the analysis without it
      console.error("Failed to fetch topology data:", topoError);
      return c.json({
        data: {
          ...parsedData,
          dreamRecordId,
...
// D2: LRU cache-aware embedding endpoint
aiRouter.post('/embed', async (c) => {
  try {
    ...
    const { dreamText, dreamId } = parsed.data;

    // D2: Check LRU cache first
    const cacheKey = `embed:${dreamText}`;
    const cachedEmbedding = embeddingCache.get(cacheKey);
    if (cachedEmbedding) {
      return c.json({ data: { embedding: cachedEmbedding, cached: true } });
    }
    ...
    return c.json({ data: { embedding, cached: false } });
  } catch (error: any) {
...
// D2: Improved match endpoint with limit parameter and LRU cache
aiRouter.post('/match', async (c) => {
  try {
    ...
    const { targetEmbedding, topK, limit } = parsed.data;
    const effectiveLimit = limit || topK;
    const userId = getUserId(c);

    // D2: Only scan records with embeddings (already filtered by Prisma query)
    ...
    const matches = dreams.map((dream: { id: string; content: string; emotion: string; content_embedding: string | null }) => {
    ...
    matches.sort((a: { similarity: number }, b: { similarity: number }) => b.similarity - a.similarity);
...
aiRouter.post('/generate-image', async (c) => {
  try {
    ...
    const { prompt, dreamId, style } = parsed.data;
    ...
    const selectedStylePrompt = stylePrompts[style as string] || stylePrompts['surrealism'];
