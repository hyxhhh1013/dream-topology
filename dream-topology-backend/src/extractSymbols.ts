...
const openai = new OpenAI({
  ...
  // If you are using a proxy or an OpenAI-compatible API (like DeepSeek, DashScope, etc.), uncomment and change the baseURL:
...
async function extractDreamSymbols(dreamText: string) {
  ...
  try {
    const response = await openai.chat.completions.create({
      model: 'gpt-4o-mini', // 或者使用其他你配置的模型
      messages: [
        { role: 'system', content: SYSTEM_PROMPT },
        { role: 'user', content: `这是我的梦境：\n${dreamText}` }
      ],
      response_format: { type: 'json_object' },
      temperature: 0.3, // 较低的温度以保证格式稳定和分析的客观性
    });
